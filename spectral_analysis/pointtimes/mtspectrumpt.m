function [S, f, R, Serr] = mtspectrumpt(data, params, fscorr, t, options)
    ...
    %                 channels/trials; also accepts 1d array of spike times)
    %                 -- required
    %   params      - structure with fields tapers, pad, Fs, fpass, err, trialave
    %                 -- optional
    %                 tapers  :precalculated tapers from dpss or in the one
    %                          of the following forms: (1) A numeric vector
    %                          [TW K] where TW is the time-bandwidth product
    %                          and K is the number of tapers to be used
    %                          (less than or equal to 2TW-1). (2) A numeric
    %                          vector [W T p] where W is the bandwidth, T is
    %                          the duration of the data and p is an integer
    %                          such that 2TW-p tapers are used. In this form
    %                          there is no default i.e. to specify the
    %                          bandwidth, you have to specify T and p as
    %                          well. Note that the units of W and T have to
    %                          be consistent: if W is in Hz, T must be in
    %                          seconds and vice versa. Note that these units
    %                          must also be consistent with the units of
    %                          params.Fs: W can be in Hz if and only if
    %                          params.Fs is in Hz. -- The default is to use
    %                          form 1 with TW=3 and K=5.
    ...
    %                 Fs      :(sampling frequency) - optional. -- Default
    %                          1.
    %                 fpass   :(frequency band to be used in the calculation
    %                          in the form [fmin fmax])- optional. --
    %                          Default all frequencies between 0 and Fs/2
    %                 err     :(error calculation [1 p] - Theoretical error
    %                          bars; [2 p] - Jackknife error bars [0 p] or 0
    %                          - no error bars) - optional. -- Default 0.
    %                 trialave:(average over channels/trials when 1, don't
    %                          average when 0) - optional. -- Default 0
    ...
    if trialave
        S = squeeze(mean(S, 2)); Msp = mean(Msp);
    end
